...
#define GL_CHECK(call)                                                                             \
    do {                                                                                           \
        ...
        if (err != GL_NO_ERROR) {                                                                  \
            nm::log(                                                                               \
                nm::LOG_ERROR,                                                                     \
                "GL error %s at %s (%u): " #call "\n",                                             \
                nm::get_gl_error_string(err),                                                      \
                __FILE__,                                                                          \
                __LINE__);                                                                         \
        }                                                                                          \
    } while (0)
...
struct shader_program {
    ...
    nm_ret init(
        const res_t* vert_src,
        const res_t* geom_src,
        const res_t* frag_src,
        const res_t* comp_src,
...
{
    GL_CHECK(glGetIntegerv(GL_UNIFORM_BUFFER_OFFSET_ALIGNMENT, &gl_ubo_alignment));
    GL_CHECK(
        glGetIntegeri_v(GL_MAX_COMPUTE_WORK_GROUP_COUNT, 0, &gl_max_compute_work_group_count[0]));
    GL_CHECK(
        glGetIntegeri_v(GL_MAX_COMPUTE_WORK_GROUP_COUNT, 1, &gl_max_compute_work_group_count[1]));
    GL_CHECK(
        glGetIntegeri_v(GL_MAX_COMPUTE_WORK_GROUP_COUNT, 2, &gl_max_compute_work_group_count[2]));
}
